...
void riis(float scale){
    PVector a = new PVector(scale,0,0);
    PVector b = new PVector(0,scale,0);
    PVector c = new PVector(0,0,scale);
    // console.log(a)
...
void draw() {
    ...
    float ratio = F/Nframe;
    float phase = ratio*3.14159;
    float rot = (1-cos(phase))/2;
    float d = 23 * pow(1.025,F);
    float pers = radians(130);
    float scale = 7;
    strokeWeight(scale*16/7);
    perspective(pers/pow(1.025,F), 1.0, 1.0, d*2);
    camera(0, 0, d, 0,0,0, 0,1,0);
    rotateY(radians((540+35)*rot));
    rotateX(radians((360+90+45)*rot));
    riis(scale);
